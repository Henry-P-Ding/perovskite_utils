...
class NumberFormatter():
    ...
    def get_constant_length(num_data, width, precision, alignment="right"):
        ...
        format_string = f"""{{0: {">" if alignment == "right" else "<"}{width}.{precision}f}}"""
        try:
...
class PWscfRelaxOutputReaderState(FileReaderState):
...
class RelaxIdleReaderState(PWscfRelaxOutputReaderState):
    def enter(self):
        pass
...
class RelaxNameReaderState(PWscfRelaxOutputReaderState):
    def enter(self):
...
class RelaxCellParametersReaderState(PWscfRelaxOutputReaderState):
    def __init__(self, file_reader):
    ...
    def execute(self):
        ...
        if len(non_empty_tokens) > 0:
            ...
            if non_empty_tokens[0] == "number of electrons":
                ...
                lattice_vec = list(map(lambda component : component * self._lattice_parameter, lattice_vec))
...
class RelaxPositionsReaderState(PWscfRelaxOutputReaderState):
    def __init__(self, file_reader):
        super().__init__(file_reader)
...
class CIFCellParametersReaderState(CIFReaderState):
    ...
    def exit(self):
        if not all(map(lambda var : var in ["a", "b", "c"], self._cell_lengths.keys())):
            raise RuntimeError(f"Not enough cell lengths read, only {self._cell_lengths.keys()}. \
            Need a, b, c.")
        elif not all(map(lambda var : var in ["alpha", "beta", "gamma"], self._cell_angles.keys())):
            raise RuntimeError(f"Not enough cell angles read, only {self._cell_angles.keys()}. \
...
class FileReader(ABC):
    ...
    def __init__(self, reader_states, start_state_name, read_path):
        ...
        if not all(map(lambda s : isinstance(s, FileReaderState), self._reader_states.values())):
            raise ValueError(f"read_states argument '{reader_states}' contains classes not of \
                             type FileReaderState.")
        self._current_str = ""  # current string being proceessed. Usually this occurs line by line.
    ...
    def __init__(self, read_path):
        super().__init__(
            reader_states={
                ...
                "cell_parameters": RelaxCellParametersReaderState(self)
            },
            start_state_name="idle",
            read_path=read_path
        )
...
class CIFFileWriter(StructureFileWriter):
    ...
    def __serialize_structure(self):
        if len(self._cell_angles) == 0 or self._cell_volume is None or len(self._cell_lengths) == 0:
        ...
        # cell parameeters
        formatted_cell_lengths = NumberFormatter.get_constant_length(self._cell_lengths, 15, 6)
        formatted_cell_angles = NumberFormatter.get_constant_length(self._cell_angles, 15, 6)
        formatted_cell_volume = NumberFormatter.get_constant_length(self._cell_volume, 15, 6)
...
_cell_volume      {formatted_cell_volume}\n"""
        ...
        formatted_pos = [NumberFormatter.get_constant_length(atom.pos, 10, 6) for atom in 
                         self.structure.atoms]
        formatted_atoms = ""
        for i, pos in enumerate(formatted_pos):
            label = self.structure.atoms[i].label
            formatted_atoms += f"    {self._numbered_labels[i]}{pos[0]}{pos[1]}{pos[2]} {label}\n"
...
class PWscfCalculation():
    def __init__(self, structure, calculation, pseudo_dir, ecut_wfc, ecut_rho, occupations, pseudos, forces, k_points_mode, k_points, out_prefix="out_", verbosity="low", etot_conv_thr=None, forc_conv_thr=None, restart_mode=None, max_seconds=None, ibrav=0, nosym=None, noncolin=None, lspinorb=None, nbnd=None, conv_thr=None, mixing_beta = None, masses=None):
        ...
        if not len(self.forces) == len(self.structure.atoms):
            raise ValueError(f"len of forces ({len(self.forces)} not equal to len of structure.atoms {len(self.structure.atoms)})")
        if not sorted(self.unique_labels) == sorted(self.pseudos.keys()):
                raise ValueError(f"keys of pseudos, {self.pseudos.keys()} must match the unique labels of structure, {self.unique_labels}.")
        ...
        if self.masses is None:
            ...
            if not sorted(self.unique_labels) == sorted(self.masses.keys()):
                raise ValueError(f"keys of masses, {self.masses.keys()} must match the unique labels of structure, {self.unique_labels}.")

        if self.ibrav != 0 and self.structure.lattice_vec is not None:
            raise ValueError(f"Input ibrav is {self.ibrav} but lattice vectors are specified in {self.structure.lattice_vec}. \
...
'''
            if self.pw_scf_calc.nosym is not None:
                system_str += f"  nosym = {self.__format_bool(self.pw_scf_calc.nosym)}\n"
            if self.pw_scf_calc.noncolin is not None:
                system_str += f"  noncolin = {self.__format_bool(self.pw_scf_calc.noncolin)}\n"
            if self.pw_scf_calc.lspinorb is not None:
                system_str += f"  lspinorb = {self.__format_bool(self.pw_scf_calc.lspinorb)}\n"
            ...
            if self.pw_scf_calc.conv_thr is not None:
                electrons_str += f"  conv_thr = {self.__format_sci_not(self.pw_scf_calc.conv_thr)}\n"
            if self.pw_scf_calc.mixing_beta is not None:
                electrons_str += f"  noncolin = {self.__format_sci_not(self.pw_scf_calc.mixing_beta)}\n"
            electrons_str += "/\n"

            ions_str = "&IONS\n/\n" if self.pw_scf_calc.calculation in ["relax", "vc-relax"] else ""
    ...
    def __format_cards(self):
        ...
        if self.structure.coordinate_mode == CoordinateModes.FRACTIONAL:
            ...
            raise ValueError(f"structure.coordinate_mode has invalid value of '{self.structure.coordinate_mode}'")
...
{''.join(NumberFormatter.get_constant_length(self.pw_scf_calc.forces[i], 2, 0))}\n"
        ...
        k_points_str = f"K_POINTS {self.pw_scf_calc.k_points_mode}\n" + self.pw_scf_calc.k_points + "\n"
...
"""
        ...
        self._cards = atomic_species_str + atomic_positions_str + k_points_str + cell_parameters_str
