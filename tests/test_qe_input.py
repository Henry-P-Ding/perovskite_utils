...
class TestSystemNamelist(unittest.TestCase):
    def test_write_to_cif(self):
        qe_reader = PWscfInputReader("tests/test_qe.relax.in")
        qe_reader.read_file()
        ...
        cif_writer = CIFFileWriter("tests/test_qe.cif", qe_reader.structure)
        cif_writer.write_file()

    def test_relax_read(self):
        qe_reader = PWscfRelaxOutputReader("tests/test_relax_output.out")
        qe_reader.read_file()
        cif_writer = CIFFileWriter("tests/test_relax_output.cif", qe_reader.structure)
        cif_writer.write_file()

    def test_cif_read(self):
        cif_reader = CIFReader("tests/test_qe.cif")
        cif_reader.read_file()
        cif_writer = CIFFileWriter("tests/test_qe_read_then_write.cif", cif_reader.structure)
        cif_writer.write_file()
    ...
    def test_relax_input(self):
        ...
        qe_writer = PWscfInputWriter("tests/qe_written.relax.in", cif_reader.structure, calculation)
